...
intro_text = "Welcome to mialoo - a learning enhancement tool creating mind maps out of lecture slides using the power of Deep Learning. A mind map functions as a multi-sensory educational tool using visual and spatial information. The goal is to display important concepts and their relation in the form of a network which may facilitate the comprehension, memorization and application of ideas. Concepts and ideas are represented by labelled nodes. Related nodes are connected to each other. Choose a lecture in the sidebar!"
...
lecture = st.sidebar.selectbox(
    "Choose a Lecture",
    ("Word2vec", "Classification", "Basis Norm", "Timeseries-AR-MA","Introduction to Databases"))

#Load Data
filename = lecture
df_keywords_orig = pd.read_pickle("df/"+filename+".pkl")
...
st.write("With a click of a button, mialoo will create a mind map in seconds! Click on the concepts for more informations! Mialoo will also search related videos for you!!")
